...
class Inventaris
{
  ...
  private:
    // m_product_init digunakan sebagai nilai initializer kepada m_products_data
    // Yang memiliki nilai default untuk setiap tipe yang ada
    ...
    // mencari semua elemen non-default di m_products_data Dan menggunakan if
    // untuk memastikan bahwa nilai id belum digunakan
    ...
    // Ini memastikan bahwa jika nilai kosong maka akan langsung dikirimkan 0
    // Ini ditujukan untuk menghindari segfault karena indexing
    //
    ...
    // hasil recursive dengan index - 1 hingga index mencapai 0 dimana recursive
    // akan diakhiri
    ...
    // Maka 'false' akan dikembalikan untuk menandakan bahwa ada nilai dan array
    // tidak kosong, jika kosong maka 'true' akan dikembalikan
    bool is_fully_empty()
    {
    ...
    // Kemudian method string find digunakan untuk mencari substring pada string
    // Jika tidak ditemukan maka std::stirng::npos akan dikembalikan
    // Oleh karena itu if menggunakan != std::string::npos
    //
    // std::string::npos adalah sebuah nilai constant yang digunakan method
    // string seperti find dimana jika tidak ditemukan maka std::string::npos
    // akan dikembalikan untuk menjadi penanda bahwa string tidak ditemukan
    //
    ...
    // Fungsi ini memiliki tujuan dan logika yang sama dengan 'search_by_name'
    // Perbedaannya hanyalah fungsi ini menggunakan id
    ...
    // Yang kemudian akan dilakukan looping dan is_empty check dilakukan untuk
    // Melewati nilai default
    //
    // Looping akan memiliki counter hingga 10 dimana pada nilai ke 10
    // Looping akan dihentikan sementara untuk menanyakan apakah pengguna ingin
    // melanjutkan Ini ditujukan untuk menghindari printing yang berlebihan
    ...
    // Pada public fungsi untuk menampilkan dan memproses menu akan diberikan
  public:
    // Constructor dari class
    Inventaris()
    {
	// Initialisasi semua element pada 'm_products_data' menggunakan nilai
	// default
	for (int i = 0; i < MAX_SIZE; i++) {
    ...
    // Karena pointer ini akan disimpan pada array maka tidak dilakukan delete
    // Pada akhir fungsi, ini ditujukan untuk menghindari error karena
    // penghapusan pointer
    //
    // Input akan dilakukan untuk setiap input
    // Kemudian for loop akan mencari element yang memiliki nilai default untuk
    // di reassign / dipasangkan nlai
    ...
    // Setelah dipilih pengguna akan diminta untuk memasukan data id atau nama
    // Yang kemudian akan dikirim ke fungsi 'search_by_id' / 'search_by_name'
    // Sesuai dengan pilihan
    // Pengguna juga diberikan pilihan untuk keluar jika ragu
    // atau ingin mencari nilai data terlebih dahulu
    ...
    // Sama seperti fungsi menu lainnya cek nilai akan dilakukan terlebih dahulu
    // Kemudian jika ada nilai fungsi recursive akan dijalankan dengan index
    // MAX_SIZE - 1, ini untuk memastikan bahwa index tidak keluar dari batas
    ...
    // Jika ada nilai yang kosong / default maka nilai tersebut akan dilewati
    // Jika ditemukan nilai non-default maka nilai stock akan dicheck
    // Untuk mencari apakah stock sudah masuk ke <= 5 atau tidak
    ...
    // Pertama dilakukan cek apakah nilai kosong, kemudian pengguna diberikan
    // pilihan untuk menggunan metode ascending(Kecil -> Besar)
    // dan Descending(Besar -> Kecil)
    // Kemudian akan dilakukan printing data setelah sorting
    // Fungsi ini memiliki efek samping pada ascending dimana nilai default akan
    // Berada di depan array, ini dikarenakan bagaimana cara nilai kosong struct
    // bekerja dimana nilai default dari stock adalah 0
    // yang membuat nilai default
    // Selalu menjadi nilai paling kecil
    ...
    // Pertama dilakukan cek inisialisasi, kemudian pengguna akan dimintai id
    // Jika sudah maka akan dilakukan pencarian
    // jika nilai ditemukan maka akan dimintai nilai baru
    // Nilai yang bisa diperbarui hanyalah nama, kategori, harga, dan stock
    // Setelah itu akan dilakukan reassign pada pointer data sesuai dengan field
    // masing - masing
